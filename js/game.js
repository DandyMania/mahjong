...
const P_POS = [
  [], [[30,40]], [[30,22],[30,58]], [[41,12],[19,40],[41,68]],
  [[19,22],[41,22],[19,58],[41,58]],
  [[19,16],[41,16],[30,38],[19,60],[41,60]],
  [[19,14],[41,14],[19,40],[41,40],[19,66],[41,66]],
  [[15,10],[30,10],[45,10],[19,37],[41,37],[19,64],[41,64]],
  [[19,12],[41,12],[19,33],[41,33],[19,54],[41,54],[19,70],[41,70]],
  [[16,10],[30,10],[44,10],[16,37],[30,37],[44,37],[16,64],[30,64],[44,64]],
];
...
function souzuSVG(n) {
  ...
  if(n===3) {
    ...
    return `<svg viewBox="0 0 60 80" width="100%" height="100%">
      ${gourd(40,4,w,h,GR)}${gourd(20,30,w,h,RD)}${gourd(40,56,w,h,GR)}
    </svg>`;
  }
  if(n===5) {
    const w=12,h=20;
  ...
  if(n===7) {
    const w=11,h=16,L=17,R=43;
    return `<svg viewBox="0 0 60 80" width="100%" height="100%">
      ${gourd(L,8,w,h,GR)}${gourd(L,28,w,h,GR)}${gourd(L,48,w,h,GR)}
      ${gourd(R,4,w,h,GR)}${gourd(R,21,w,h,RD)}${gourd(R,38,w,h,GR)}${gourd(R,55,w,h,GR)}
    </svg>`;
  }
  ...
  for(let i=0;i<n;i++){
    ...
    const isRed=(n===8&&(i===3||i===5))||(n===9&&i===4);
...
const TRANS_TIPS = [
  ...
  'ライフ1のピンチで轟盲牌スキルを持ってると自動発動！正解で3倍スコア',
  'ショップで時間延長を買うと余裕が生まれるよ。序盤の優先投資に！',
  '「連撃」スキルがあればミスしてもコンボが-1のみ。スコア狙いに必須',
